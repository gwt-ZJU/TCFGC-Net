...
def main(left, top, right, bottom, zoom, style='s', server="Google China"):
    """
...
def downloader_Img(Save_file,Cluster_ID,left_top_x,left_top_y,right_bottom_x, right_bottom_y,zoom=20,style='s',server="Google"):
    urls = get_urls(left_top_x, left_top_y, right_bottom_x, right_bottom_y, zoom, server, style)
    # Download tiles
    datas = download_tiles(urls)
    # Combine downloaded tile maps into one map
    outpic = merge_tiles(datas, left_top_x, left_top_y, right_bottom_x, right_bottom_y, zoom)
    outpic = outpic.convert('RGB')
    save_path = os.path.join(Save_file,'neg'+str(int(Cluster_ID))+'.jpg')
    outpic.save(save_path)
...
if __name__ == '__main__':
    ...
    加载和保存代爬取的文件
    """
    Cluster_DataFrame_path = '../../Data/待爬取卫星图信息/Non_Hotpots.csv'
    Cluster_DataFrame = pd.read_csv(Cluster_DataFrame_path)
    Save_file = 'Non_Hotpots_Satellite'
    for index,row in tqdm(Cluster_DataFrame.iterrows(),total=len(Cluster_DataFrame)):
        try:
            Cluster_ID = row['Cluster_ID']
            img_path = os.path.join(Save_file,'neg'+str(int(Cluster_ID))+'.jpg')
            left_top_x,left_top_y = row['left_top_x'],row['left_top_y']
            if os.path.isfile(img_path):
                continue
            right_bottom_x, right_bottom_y = row['right_bottom_x'],row['right_bottom_y']
            downloader_Img(Save_file,Cluster_ID,left_top_x,left_top_y,right_bottom_x,right_bottom_y)
        except:
            print(index)



    # main(120.31726757823225, 30.154818330029272, 120.31819198061703, 30.15435532215823, 20, '../Data/test.tif',
    #      server="Google")
    # end_time = time.time()
